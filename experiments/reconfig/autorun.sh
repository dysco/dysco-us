...
init_mbs() {
        if [ $HOSTNAME != $HOST_MBS ]; then
			...
			$REXEC SECURE=$2 DYSCO=$1 /u/ronaldof/udysco/bessctl/bessctl run reconfig-mbs\r\"
                        $REXEC exit\r\"
                "
        else
                expect -c "
		       	...
		       	expect \"\\\\$\"
			send \"SECURE=$2 DYSCO=$1 /u/ronaldof/udysco/bessctl/bessctl run reconfig-mbs\r\"
                        expect \"\\\\$\"
                        send \"exit\r\"
                "
        fi
...
init_server() {
	if [ $HOSTNAME != $HOST_SERVER ]; then
                remote_exec $HOST_SERVER "
                        ...
                        expect \"\\\\#\"
                        $REXEC /u/ronaldof/udysco/bessctl/bessctl daemon start\r\" 
                        $REXEC SECURE=$2 DYSCO=$1 /u/ronaldof/udysco/bessctl/bessctl run reconfig-server\r\"
                        $REXEC exit\r\"
                "
        else
	        expect -c "
                        ...
                        send \"/u/ronaldof/udysco/bessctl/bessctl daemon start\r\" 
                        expect \"\\\\$\"
                        send \"SECURE=$2 DYSCO=$1 /u/ronaldof/udysco/bessctl/bessctl run reconfig-server\r\"
                        expect \"\\\\$\"
                        send \"exit\r\"
                "
        fi
...
init_client() {
        if [ $HOSTNAME != $HOST_CLIENT ]; then
                remote_exec $HOST_CLIENT "
                        ...
                        expect \"\\\\#\"
                        $REXEC /u/ronaldof/udysco/bessctl/bessctl daemon start\r\" 
                        $REXEC SECURE=$2 DYSCO=$1 /u/ronaldof/udysco/bessctl/bessctl run reconfig-client\r\"
                        $REXEC exit\r\"
                "
        else
		expect -c "
                        ...
                        send \"/u/ronaldof/udysco/bessctl/bessctl daemon start\r\" 
                        expect \"\\\\$\"
                        send \"SECURE=$2 DYSCO=$1 /u/ronaldof/udysco/bessctl/bessctl run reconfig-client\r\"
                        expect \"\\\\$\"
                        send \"exit\r\"
                "
        fi
