...
if [ $SERVER_AND_CLIENT == 1 ]; then
	if [ $HOSTNAME != $HOST_SERVER ]; then
                remote_exec $HOST_SERVER "
		       	...
		       	$REXEC DYSCO=$1 SECURE=$2 /u/ronaldof/udysco/bessctl/bessctl run requests-client-and-server\r\"
                        $REXEC exit\r\"
                "
        else
                expect -c "
			...
			expect \"\\\\$\"
			send \"DYSCO=$1 SECURE=$2 /u/ronaldof/udysco/bessctl/bessctl run requests-client-and-server\r\"
                        expect \"\\\\$\"
			send \"exit\r\"
                "
        fi
	...
	if [ $HOSTNAME != $HOST_SERVER ]; then
                remote_exec $HOST_SERVER "
                        ...
                        expect \"\\\\#\"
                        $REXEC /u/ronaldof/udysco/bessctl/bessctl daemon start\r\"
		       	$REXEC DYSCO=$1 SECURE=$2 /u/ronaldof/udysco/bessctl/bessctl run requests-server\r\"
                        $REXEC exit\r\"
                "
        else
                expect -c "
                        ...
                        send \"/u/ronaldof/udysco/bessctl/bessctl daemon start\r\"
			expect \"\\\\$\"
			send \"DYSCO=$1 SECURE=$2 /u/ronaldof/udysco/bessctl/bessctl run requests-server\r\"
                        expect \"\\\\$\"
			send \"exit\r\"
                "
	fi
...
if [ $SERVER_AND_CLIENT == 0 ]; then
	if [ $HOSTNAME != $HOST_CLIENT ]; then
                remote_exec $HOST_CLIENT "
                        ...
                        expect \"\\\\#\"
                        $REXEC /u/ronaldof/udysco/bessctl/bessctl daemon start\r\"
		       	$REXEC DYSCO=$1 SECURE=$2 /u/ronaldof/udysco/bessctl/bessctl run requests-client\r\"
                        $REXEC exit\r\"
                "
        else
                expect -c "
                        ...
                        send \"/u/ronaldof/udysco/bessctl/bessctl daemon start\r\"
			expect \"\\\\$\"
			send \"DYSCO=$1 SECURE=$2 /u/ronaldof/udysco/bessctl/bessctl run requests-client\r\"
                        expect \"\\\\$\"
			send \"exit\r\"
                "
        fi
...
init_mbs() {
        if [ $HOSTNAME != $HOST_MBS ]; then
                remote_exec $HOST_MBS "
                        ...
                        expect \"\\\\#\"
                        $REXEC /u/ronaldof/udysco/bessctl/bessctl daemon start\r\"
		       	$REXEC DYSCO=$1 SECURE=$2 /u/ronaldof/udysco/bessctl/bessctl run requests-mbs\r\"
                        $REXEC exit\r\"
                "
        else
                expect -c "
                        ...
                        send \"/u/ronaldof/udysco/bessctl/bessctl daemon start\r\"
		       	expect \"\\\\$\"
			send \"DYSCO=$1 SECURE=$2 /u/ronaldof/udysco/bessctl/bessctl run requests-mbs\r\"
                        expect \"\\\\$\"
                        send \"exit\r\"
                "
        fi
...
echo -n "[$(TZ=America/New_York date +"%T.%6N")] Starting virtual machines with Dysco... "
{
