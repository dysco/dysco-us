...
static const struct rte_eth_conf default_eth_conf() {
  ...
  ret.rx_adv_conf.rss_conf.rss_key = nullptr;
  ret.rx_adv_conf.rss_conf.rss_key_len = 40;
  ret.rx_adv_conf.rss_conf.rss_hf = ETH_RSS_IP | ETH_RSS_UDP | ETH_RSS_TCP;

  return ret;
}

void PMDPort2::InitDriver() {
...
static CommandResponse find_dpdk_port_by_pci_addr(const std::string &pci,
  ...
  if (port_id == DPDK_PORT_UNKNOWN) {
    return CommandFailure(ENODEV, "Cannot attach PCI device");
  }
...
CommandResponse PMDPort2::Init(const bess::pb::PMDPort2Arg &arg) {
...
int PMDPort2::UpdateConf(const Conf &conf) {
...
void PMDPort2::CollectStats(bool reset) {
...
int PMDPort2::RecvPackets(queue_t qid, bess::Packet **pkts, int cnt) {
  return rte_eth_rx_burst(dpdk_port_id_, qid, (struct rte_mbuf **)pkts, cnt);
}

int PMDPort2::SendPackets(queue_t qid, bess::Packet **pkts, int cnt) {
...
Port::LinkStatus PMDPort2::GetLinkStatus() {
...
ADD_DRIVER(PMDPort2, "pmd_port2", "DPDK poll mode driver")
