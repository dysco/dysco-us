...
PlainPacketPool::PlainPacketPool(size_t capacity, int socket_id)
  ...
  size_t chunk_size, align;
  size_t size = rte_mempool_ops_calc_mem_size(pool_, pool_->size, page_shift, &chunk_size, &align);
...
BessPacketPool::BessPacketPool(size_t capacity, int socket_id)
  ...
  while (pool_->populated_size < pool_->size) {
    ...
    size_t chunk_size, align;
    size_t bytes = rte_mempool_ops_calc_mem_size(pool_, deficit, page_shift, &chunk_size, &align);
